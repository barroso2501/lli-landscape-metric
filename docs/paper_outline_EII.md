...
Last updated: 2026-03-30 (rev. 2 — section 3.7.1 expanded)
...
A formal transect-based metric of landscape interface connectivity derived from regular grids.
A biome-scale, four-decade characterization of Area–EII joint distributions and their structural breaks.
A systematic treatment of the Modifiable Areal Unit Problem (MAUP) as quantifiable sampling variance rather than an unavoidable limitation.
...
Ai(t)=∑p∈Ci1[rp(t)=1]∣Ci∣A_i(t) = \frac{\sum_{p \in C_i} \mathbf{1}[r_p(t) = 1]}{|C_i|}Ai​(t)=∣Ci​∣∑p∈Ci​​1[rp​(t)=1]​
where CiC_i
Ci​ is the set of pixels within cell ii
i, rp(t)r_p(t)
rp​(t) is the raster value of pixel pp
p at time tt
t, and ∣Ci∣|C_i|
∣Ci​∣ is the total number of valid pixels.

Edge Interception Index (EII):
wi(t)=Linat(t)Piobs(t)w_i(t) = \frac{L_i^{\text{nat}}(t)}{P_i^{\text{obs}}(t)}wi​(t)=Piobs​(t)Linat​(t)​
where Linat(t)L_i^{\text{nat}}(t)
Linat​(t) is the length (in pixels) of the cell perimeter that intersects natural habitat at time tt
t, and Piobs(t)P_i^{\text{obs}}(t)
Piobs​(t) is the total observable perimeter length (excluding pixels with nodata values).


wi(t)∈[0,1]w_i(t) \in [0, 1]
wi​(t)∈[0,1]: 0 = perimeter entirely surrounded by non-natural pixels; 1 = perimeter entirely surrounded by natural habitat.

For border cells of the study domain: Piobs<PiP_i^{\text{obs}} < P_i
Piobs​<Pi​ (total perimeter); completeness ratio Piobs/PiP_i^{\text{obs}} / P_i
Piobs​/Pi​ is reported as a data quality flag.
...
wi(t)w_i(t)
wi​(t) is thus the hit rate along the perimeter transect — directly analogous to the cover estimator of Canfield (1941).
...
Area metric (AiA_i
Ai​) computed for the same period (already available).
...
Output per cell: year(s) of structural break in EII (tw∗t^*_w
tw∗​) and in Area (tA∗t^*_A
tA∗​).

Derived variable: temporal lag δi=tw∗−tA∗\delta_i = t^*_w - t^*_A
δi​=tw∗​−tA∗​ (positive = EII changes before Area; negative = Area changes before EII).

Aggregation: frequency distribution of δ\delta
δ across cells; spatial map of δ\delta
δ.


3.9 Area × EII joint distribution analysis

Discretization: both AiA_i
Ai​ and wiw_i
wi​ binned into five 20% classes → 5×5 frequency matrix per time period.
...
Correlation between Ai(t)A_i(t)
Ai​(t) and wi(t)w_i(t)
wi​(t) across cells and time periods.
...
Frequency distribution of temporal lag δi\delta_i
δi​.

Proportion of cells where EII changepoint precedes Area changepoint (potential early-warning signal).
Spatial map of δi\delta_i
δi​ across the Cerrado — regional patterns of lead/lag.
...
Relationship to graph-based connectivity: EII weights (wijw_{ij}
wij​) can be used directly as edge weights in a spatial graph — connection to future work.
...
Figures (planned)
FigureContentSource analysisFig. 1Conceptual diagram: hexagonal cell, perimeter as transect, pixel contactsIllustrationFig. 2Example cells showing Area × EII decoupling (visual tiles)MapBiomas tilesFig. 3Area × EII scatter + 5×5 matrices for 1985 and 2020 + difference matrixSection 4.1, 4.3Fig. 4Maps of AiA_i
Ai​, wiw_i
wi​, and decoupling type across the Cerrado
Section 4.3Fig. 5Changepoint analysis: distribution of δi\delta_i
δi​; spatial map of lead/lag
Section 4.2Fig. 6MAUP sensitivity: jitter CV map; hex vs. square comparison; scale comparisonSection 4.4
